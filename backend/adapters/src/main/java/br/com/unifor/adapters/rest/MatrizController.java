...
public class MatrizController {
    ...
    // Nova visão do ALUNO (casando com o front: /matriz/aluno?cursoId=...)
    @GET
    @Path("/aluno")
    public MatrizAlunoResponseDTO listarAluno(@QueryParam("cursoId") Long cursoId) {
        if (cursoId == null) {
    ...
    // Nova visão do PROFESSOR (casando com o front: /matriz/professor?professorId=...)
    @GET
    @Path("/professor")
    public MatrizProfessorResponseDTO listarProfessor(@QueryParam("professorId") Long professorId) {
        if (professorId == null) {
    ...
    // LEGADO (opcional): mantém compatibilidade com /Matriz?cursoId=...
    @GET
    @Path("/legacy")
    public List<MatrizItemDTO> listarLegacy(@QueryParam("cursoId") Long cursoId) {
        if (cursoId == null) {
            throw new BadRequestException("Parâmetro cursoId é obrigatório");
        }
